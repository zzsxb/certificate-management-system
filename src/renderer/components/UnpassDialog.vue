    ...
    <div class="dialog-container">
      ...
      >
        ...
        <el-table-column prop="missing_certs" label="应考未考证书" min-width="200" show-overflow-tooltip>
          <template #default="{ row }">
            <span class="missing-cert">{{ row.missing_certs || '-' }}</span>
          </template>
        </el-table-column>
...
const exportData = () => {
  ...
  const exportData = filteredEmployees.value.map(e => ({
    ...
    'PS岗位': e.ps_position || '',
    '已持证书': e.held_certs || '',
    '应考未考证书': e.missing_certs || ''
  }))

  const ws = XLSX.utils.json_to_sheet(exportData)
  const wb = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(wb, ws, '应考未考人员')
  XLSX.writeFile(wb, `应考未考人员_${new Date().toISOString().slice(0, 10)}.xlsx`)
